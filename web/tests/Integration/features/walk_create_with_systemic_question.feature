Feature: Testing walk create resource without systemic question
  ...
  @api @walkCreate
  Scenario: I can request /api/walks/create with a team who has isWithSystemicQuestion disabled
...
#    And print last response
    ...
    And I can find the following walks in database:
      | name            | isWithSystemicQuestion |
      | This is my Walk | <false>          |

  @api @walkCreate
  Scenario: I can request /api/walks/create with a team who has isWithSystemicQuestion enabled
    Given I am authenticated against api as "karl@gmx.de"
    When I send an api platform "POST" request to "/api/walks/create" with parameters:
      | key             | value                 |
      | team            | teamIri<CA>           |
