Feature: Testing teamNames resource
    ...
    @api @teamNames
    Scenario: I can request /api/walks/team_names as a not authenticated user and an auth error will occur
        When I send a GET request to "/api/walks/team_names"
...
#    And print last JSON response
    ...
    @api @teamNames
    Scenario: I can request /api/walks/team_names as authenticated user and get a restricted result
...
#    And print last JSON response
        And the JSON nodes should be equal to:
            | totalItems         | 2        |
            | member[0].teamName | CA       |
            | member[1].teamName | Westhang |
