Feature: Testing guestNames resource
    ...
    Background:
        ...
        Given the following walks exists:
            | name        | team     | guestNames                |
            | Spaziergang | CA       | array<Opa Manfred, Karla> |
            | Gogo        | Westhang | array<Karla>              |
            | Gamescon    | Gamers   | array<Karla>              |

    @api @guestNames
    Scenario: I can request /api/walks/guest_names as a not authenticated user and an auth error will occur
        When I send a GET request to "/api/walks/guest_names"
...
#    And print last JSON response
    ...
    @api @guestNames
    Scenario: I can request /api/walks/guest_names as authenticated user and get a restricted result
        ...
        And the JSON nodes should be equal to:
            | totalItems      | 2           |
            | member[0].@type | GuestName   |
            | member[0].name  | Karla       |
            | member[1].name  | Opa Manfred |
